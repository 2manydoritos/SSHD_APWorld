...
class CommonContext:
    ...
    def _is_socket_open(self) -> bool:
        """Check if websocket connection is open (compatible with websockets 16.0+)."""
        if not self.server or not self.server.socket:
            return False
        try:
            # websockets 16.0+ uses state enum
            return self.server.socket.state.name == 'OPEN'
        except AttributeError:
            # Fallback for older websockets versions
            if hasattr(self.server.socket, 'open'):
                return self.server.socket.open
            return not getattr(self.server.socket, 'closed', True)
    ...
    async def update_death_link(self, death_link: bool):
        ...
        if death_link:
            ...
            self.tags -= {"DeathLink"}
        if old_tags != self.tags and self._is_socket_open():
